...
public static class Converter
{
    ...
    public static string AddressToString(byte[] decoded)
    {
        ...
        {
            var padded = new byte[_constants["sizes"]["symbolAddressDecoded"] + 1];
    ...
    public static byte[] Xor2(byte[] currentMetadataValueBytes, byte[] newMetadataValueBytes)
    {
        var length = Math.Max(currentMetadataValueBytes.Length, newMetadataValueBytes.Length);
        var currentList = currentMetadataValueBytes.ToList();
        for (var i = currentList.Count; i < length; i++) currentList.Add(0);
        var newList = newMetadataValueBytes.ToList();
        for (var i = newList.Count; i < length; i++) newList.Add(0);
        var metadataNewArray = new byte[length];
        for (var i = 0; i < length; i++) metadataNewArray[i] = (byte) (currentList[i] ^ newMetadataValueBytes[i]);
        return metadataNewArray;
    }
