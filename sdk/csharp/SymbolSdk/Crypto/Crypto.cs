...
public static class Crypto
{
    ...
    */
    ...
    public static string Encode(byte[] senderPrivateKey, string recipientPub, string msg, bool isHexString = false, byte[]? iv = null)
    {
        iv ??= RandomBytes(12);
        return _Encode(senderPrivateKey, Converter.HexToBytes(recipientPub), isHexString ? msg : Converter.Utf8ToHex(msg), iv);
    }

    public static string Encode(string senderPrivateKey, byte[] recipientPub, string msg, bool isHexString = false, byte[]? iv = null)
    {
        iv ??= RandomBytes(12);
        return _Encode(Converter.HexToBytes(senderPrivateKey), recipientPub, isHexString ? msg : Converter.Utf8ToHex(msg), iv);
    }

    public static string Encode(byte[] senderPrivateKey, byte[] recipientPub, string msg, bool isHexString = false, byte[]? iv = null)
    {
        iv ??= RandomBytes(12);
        return _Encode(senderPrivateKey, recipientPub, isHexString ? msg : Converter.Utf8ToHex(msg), iv);
    }
    
    public static string Encode<T, U>(T senderPrivateKey, U recipientPub, string msg, bool isHexString = false, byte[]? iv = null)
        where T : ByteArray 
        where U : ByteArray
    {
        iv ??= RandomBytes(12);
