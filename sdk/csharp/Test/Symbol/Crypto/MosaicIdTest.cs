...
public class MosaicIdTest
{
	...
	public void MosaicIdTest0()
    {
		...
		var mainAd = MainFacade.Network.PublicKeyToAddress(Converter.HexToBytes(publicKey));
		var mainAddress = Converter.AddressToString(mainAd.bytes);
		var testAd = TestFacade.Network.PublicKeyToAddress(Converter.HexToBytes(publicKey));
        ...
        Assert.Multiple(() =>
        {
            Assert.That(mainAddress, Is.EqualTo(addressPublic));
            Assert.That(testAddress, Is.EqualTo(addressPublicTest));
            Assert.That(mosaicIdMain, Is.EqualTo(mosaicIdPublic));
            Assert.That(mosaicIdTest, Is.EqualTo(mosaicIdPublicTest));
        });
    }
