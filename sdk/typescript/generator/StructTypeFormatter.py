...
class StructFormatter(AbstractTypeFormatter):
	...
	@staticmethod
	def generate_ctor_args(data: dict) -> str:
		keys = ', '.join(data.keys())
		types = ', '.join([f'{k}?: {v}' for k, v in data.items()])
		return f'{{ {keys} }}\n: {{ {types} }} = {{}}'
	...
	@staticmethod
	def field_name(field, object_name='this', prefix_field=True, is_argument=False):
	...
	def generate_reserved_field(self, field):
		field_name = self.field_name(field, '', False)
		value = field.value
		return f'private {field_name}: {field.extensions.printer.get_type()} = {value}; // reserved field\n'
	...
	def get_ctor_descriptor(self):
		body = 'super();\n' if self.is_inner_transaction() or self.is_transaction() else ''
		data = {}
		for field in self.non_reserved_fields():
			arg_name = self.field_name(field, is_argument=True)
			type = self.re_name(field.extensions.printer.get_type())
			data[arg_name] = type
		...
		arguments = [self.generate_ctor_args(data)]
	...
	def generate_serialize_field(self, field):
		...
		if DisplayType.TYPED_ARRAY == field.display_type:
			serialize_field = f"this.sort();\n{field.extensions.printer.store(field_value, 'buffer')}"
			serialize_line = f'{serialize_field};{field_comment}\n'
		else:
			serialize_field = field.extensions.printer.store(field_value)
			if self.is_nullable_field(field):
				serialize_field = serialize_field.replace('.serialize', '!.serialize')
	...
	def generate_size_field(self, field):
		...
		if self.is_nullable_field(field):
			size_field = size_field.replace('.size', '!.size')
	...
	def generate_str_field(self, field):
		...
		if DisplayType.TYPED_ARRAY == field.display_type:
			...
			if self.is_nullable_field(field):
				field_to_string = field_to_string.replace('.toString', '!.toString')
			value = f'${{{field_to_string}}}'
