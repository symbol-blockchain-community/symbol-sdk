...
class EnumTypeFormatter(AbstractTypeFormatter):
	...
	def get_fields(self):
		fields = list(
			map(
				lambda e: f'static {e.name}: {self.typename} = new {self.typename}({e.value});\n',
				self.enum_type.values,
			)
		)
		fields.append('value: number = 0;\n')
		return fields
	...
	def _get_deserialize_descriptor(self, is_aligned):
		...
		return MethodDescriptor(body=body, result=f': {self.typename}')
	...
	def get_serialize_descriptor(self):
		return MethodDescriptor(body=f'return {self.int_printer.store("this.value")};', result=f': Uint8Array')
...
return keys[index];
'''
		return MethodDescriptor(method_name='static valueToKey', body=body, arguments=['value: number'], result=': string')

	def get_map_descriptor(self):
		return MethodDescriptor(method_name='static fromValue', body=f'return new {self.typename}(value);', arguments=['value: number'], result=f': {self.typename}')
	...
	def get_getter_setter_descriptors(self):
		...
		if self.enum_type.is_bitwise:
			...
			methods.append(MethodDescriptor(method_name='has', arguments=['flag: number'], body=body))
		else:
