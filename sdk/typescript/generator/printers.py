...
class IntPrinter(Printer):
	...
	def get_type(self):
		return 'bigint' if 8 == self.descriptor.size else 'number'
...
class TypedArrayPrinter(Printer):
	...
	def _get_sort_comparer(self, variable_name):
		...
		comparer = f'arrayHelpers.getValue({variable_name}.{sort_key})'
		return comparer

	def _get_sort_accessor(self, element_type=any):
		accessor = f'(e: {element_type}) => ({self._get_sort_comparer("e")})'
		return accessor
	...
	def load(self, buffer_name, is_aligned):
		...
		cast_type =  f' as {self.descriptor.field_type.element_type}[]'
		...
		if self.is_variable_size:
			...
			alignment = self.descriptor.field_type.alignment
			skip_last_element_padding = js_bool(not self.descriptor.field_type.is_last_element_padded)
			return f'arrayHelpers.readVariableSizeElements({buffer_view}, {element_type}, {alignment}, {skip_last_element_padding}){cast_type}'

		if self.descriptor.field_type.is_expandable:
			return f'arrayHelpers.readArray(view.buffer, {element_type}){cast_type}'
		...
		if self.descriptor.field_type.sort_key:
			args.append(self._get_sort_accessor(element_type))

		args_str = ', '.join(args)
		return f'arrayHelpers.readArrayCount({args_str}){cast_type}'
	...
	def store(self, field_name, buffer_name):
		...
		if self.descriptor.field_type.sort_key:
			args.append(self._get_sort_accessor(self.descriptor.field_type.element_type))
