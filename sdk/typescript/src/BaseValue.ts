...
const isNonNegative = (value: bigint | number) => {
	if ('bigint' === typeof value)
		return 0n <= value;
 ...
 */
export default class BaseValue implements ISerializable {
	size: number;
	isSigned: boolean;
	value: any;
	/**
	 ...
	 * @param {number|bigint|string} value Value.
	 * @param {boolean} isSigned \c true if the value should be treated as signed.
	 */
	constructor(size: number, value: number | bigint | string, isSigned: boolean = false) {
		/**
		 * Size of the integer.
		 * @type number
		 */
