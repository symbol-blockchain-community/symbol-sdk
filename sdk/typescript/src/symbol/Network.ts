...
import { Network as BasicNetwork } from '../Network';
import { NetworkTimestamp as BasicNetworkTimestamp, NetworkTimestampDatetimeConverter } from '../NetworkTimestamp';
 ...
 */
export class SymbolAddress extends ByteArray {
	/**
...
export class Network extends BasicNetwork {
	...
	constructor(name: string, identifier: number, epochTime: Date, generationHashSeed: Hash256) {
		super(
			...
			() => sha3_256.create(),
			(addressWithoutChecksum: any, checksum: { subarray: (arg0: number, arg1: number) => any; }) => new SymbolAddress(new Uint8Array([...addressWithoutChecksum, ...checksum.subarray(0, 3)])),
			SymbolAddress,
			NetworkTimestamp
		);
